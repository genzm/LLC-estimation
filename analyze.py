...
from lsa_common import compute_llc_theoretical_dln

def get_lambda_w(d, dl, r, alpha):
    if r + alpha > d + 1 or r + alpha > dl or r < 1 or dl < 1: return 0.0
    upper = 2 * (d + 1) - (r + alpha)
    lower = r + alpha
    term = dl + r + alpha
    if dl >= upper:
...
def plot_analysis_with_boundaries():
    ...
    for i, r in enumerate(r_samples):
        ...
        for j, alpha in enumerate(alphas):
            diff_values = []
            for dl in dl_range:
                ld = compute_llc_theoretical_dln([d_fixed, dl, d_fixed], r)
...
def get_general_lambda(h_in, h_hid, h_out, r):
    ...
    if r > min(h_in, h_out) or r > h_hid:
        return 0.0
    if r < 0 or h_hid < 1:
        return 0.0
